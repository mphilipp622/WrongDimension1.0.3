...
{
    /// <summary>
    /// Delegate for tapping into the InputManager's update cycle
    /// </summary>
    public delegate void InputManagerUpdateFunc();
    ...
    public class InputManager
    {
        ...
        /// </summary>
        private static bool mUseGamepad = false;
        ...
        /// <summary>
        /// Lerp value used for smoothing. This really shouldn't be used.
        /// </summary>
        private static float mViewLerp = 0f;
        public static float ViewLerp
        {
            get { return mViewLerp; }
            set { mViewLerp = value; }
        }
        ...
        /// </summary>
        private static float mMouseViewSensativity = 1f;
        ...
        /// <summary>
        /// Allows callers to tap into the InputManager's update cycle
        /// </summary>
        public static InputManagerUpdateFunc OnUpdate = null;
        ...
        public static Dictionary<string, List<InputAlias>> Aliases
        {
            get { return mAliases; }
        }
...
#pragma warning disable 0414
                ...
                GameObject lCoreGameObject = new GameObject("InputManagerCore", typeof(InputManagerCore));
                lCoreGameObject.hideFlags = HideFlags.HideInHierarchy;

                Core = lCoreGameObject.GetComponent<InputManagerCore>();
...
#pragma warning restore 0414
        ...
        public static void Initialize()
        {
            if (mAliases == null)
            {
                ...
                if (mPrimaryAliases == null) { mPrimaryAliases = new Dictionary<int, List<InputAlias>>(); }
                ...
                AddAlias("_MoveLeftKey", EnumInput.LEFT_ARROW);
                AddAlias("_MoveRightKey", EnumInput.RIGHT_ARROW);
                AddAlias("_MoveUpKey", EnumInput.UP_ARROW);
                AddAlias("_MoveDownKey", EnumInput.DOWN_ARROW);
        ...
        /// <returns></returns>
        public static bool IsAliasSet(string rName)
        {
            return (mAliases.ContainsKey(rName) && mAliases[rName].Count > 0);
        }
        ...
        /// <summary>
        /// Removes all aliases including default ones
        /// </summary>
        public static void RemoveAliases()
        {
            foreach (string lKey in mAliases.Keys)
            {
                mAliases[lKey].Clear();
            }
            ...
            foreach (int lKey in mPrimaryAliases.Keys)
            {
                mPrimaryAliases[lKey].Clear();
            }

            mPrimaryAliases.Clear();
        }
        ...
        /// <summary>
        /// Determines if the player can freely look around. Use IsViewingActivated,
        /// but this works for backwards compatability.
        /// </summary>
        public static bool IsFreeViewing
        {
            get { return IsViewingActivated; }
        }

        /// <summary>
        /// Determines if the player can freely look around
        /// </summary>
        public static bool IsViewingActivated
        {
            get
            {
                // If no 'view activator' is set, we must always be viewing. If it is set, check it
                bool lValue = (!mAliases.ContainsKey("_MouseViewEnable") || IsPressed("_MouseViewEnable"));

                // Check the Xbox controller if needed
                if (!lValue)
                {
#if UNITY_EDITOR_OSX || UNITY_STANDALONE_OSX
                    lValue = (mUseGamepad && UnityEngine.Input.GetAxis("MXRightStickX") != 0f);
                    if (!lValue) { lValue = (mUseGamepad && UnityEngine.Input.GetAxis("MXRightStickY") != 0f); }
#else
                    lValue = (mUseGamepad && UnityEngine.Input.GetAxis("WXRightStickX") != 0f);
                    if (!lValue) { lValue = (mUseGamepad && UnityEngine.Input.GetAxis("WXRightStickY") != 0f); }
#endif
                }
        ...
        public static int ViewActivator
        {
            get
            {
                if (mAliases == null) { return 0; }

                if (mAliases.ContainsKey("_MouseViewEnable") && mAliases["_MouseViewEnable"].Count > 0)
                {
                    return mAliases["_MouseViewEnable"][0].PrimaryID;
                }
                else
                {
                    return 0;
            ...
            set
            {
                if (value < 0)
                {
                    RemoveAlias("_MouseViewEnable");
                }
                else
                {
                    AddAlias("_MouseViewEnable", value);
                }
            }
        }
        ...
        public static float ViewX
        {
            get
            {
                ...
                if (lView != 0f) { return lView * (Time.deltaTime / 0.01666f); }
                ...
                if (lEnabled)
                {
                    ...
                    if (mViewLerp > 0f && mViewLerp < 1f) { lView = Mathf.Lerp(mOldViewX, lView, mViewLerp); }
                    mOldViewX = lView;
                }
        ...
        public static float ViewY
        {
            get
            {
                ...
                if (lEnabled)
                {
                    ...
                    if (mViewLerp > 0f && mViewLerp < 1f) { lView = Mathf.Lerp(mOldViewY, lView, mViewLerp); }
                    mOldViewY = lView;
                }
        ...
        {
            if (!mAliases.ContainsKey(rAlias))
            {
                try
                {
                    float lValue = UnityEngine.Input.GetAxis(rAlias);
                    if (lValue == 0f) { lValue = (UnityEngine.Input.GetButton(rAlias) ? 1f : 0f); }
        ...
        /// <summary>
        /// Determines if the specified button/key is pressed. This uses Unity's
        /// functionality and while it works, doesn't support our advanced features.
        /// </summary>
        /// <returns><c>true</c> if is pressed the specified rButtonID; otherwise, <c>false</c>.</returns>
        /// <param name="rUnityKey">Button ID to test</param>
        public static bool IsPressed(UnityEngine.KeyCode rUnityKey)
        {
            return UnityEngine.Input.GetKey(rUnityKey);
        }
        ...
        public static bool IsPressed(string rAlias)
        {
            if (!mAliases.ContainsKey(rAlias))
            {
                try
                {
                    bool lValue = UnityEngine.Input.GetButton(rAlias);
                    if (!lValue) { lValue = (UnityEngine.Input.GetAxis(rAlias) != 0f); }
        ...
        /// <returns><c>true</c> if is pressed the specified rButtonID; otherwise, <c>false</c>.</returns>
        /// <param name="rUnityKey">Button ID to test</param>
        public static bool IsJustPressed(UnityEngine.KeyCode rUnityKey)
        {
            return UnityEngine.Input.GetKeyDown(rUnityKey);
        }
        ...
        public static bool IsJustPressed(string rAlias)
        {
            if (!mAliases.ContainsKey(rAlias))
            {
                try
                {
                    return UnityEngine.Input.GetButtonDown(rAlias);
                }
                catch
                {
                    return false;
        ...
        /// <returns><c>true</c> if is pressed the specified rButtonID; otherwise, <c>false</c>.</returns>
        /// <param name="rUnityKey">Button ID to test</param>
        public static bool IsJustReleased(UnityEngine.KeyCode rUnityKey)
        {
            return UnityEngine.Input.GetKeyUp(rUnityKey);
        }
        ...
        public static bool IsJustReleased(string rAlias)
        {
            if (!mAliases.ContainsKey(rAlias))
            {
                try
                {
                    return UnityEngine.Input.GetButtonUp(rAlias);
                }
                catch
                {
                    return false;
        ...
        public static void Update()
        {
            ...
            else
            {
                ...
                if (mUseKeyboardBasic)
                {
                    ...
                    mState.Controls[EnumInput.PAUSE].Value = (UnityEngine.Input.GetKey(KeyCode.Pause) ? 1 : 0);
                    mState.Controls[EnumInput.PRINT].Value = (UnityEngine.Input.GetKey(KeyCode.Print) ? 1 : 0);
            ...
            // Call out to external update functions
